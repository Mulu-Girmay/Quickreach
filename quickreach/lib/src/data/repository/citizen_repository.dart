...
class CitizenRepository {
  ...
  Future<OfflineIncidentRecord> createLocalIncident({
    ...
    await _db.insertOfflineIncident(incident);
    await _db.insertSyncJob(
      SyncQueueRecord(
        queueId: _uuid.v4(),
        localId: localId,
  ...
  Future<void> refreshCachedIncident(String serverIncidentId) async {
    try {
      final response = await _dio.get(
        ...
        options: Options(
          headers: {"x-incident-token": serverIncidentId},
        ),
      );
      final incident = Map<String, dynamic>.from(
        (response.data as Map<String, dynamic>)["incident"] as Map,
      );
      final cached = CachedIncidentRecord.fromServerJson(
        incident,
        localId: String(incident["client_request_id"] ?? serverIncidentId),
        offlineCreated: incident["offline_created"] == true,
        clientCreatedAt:
            DateTime.tryParse(String(incident["client_created_at"] ?? "")) ??
            DateTime.now(),
      );
  ...
  Future<void> _processJob(SyncQueueRecord job) async {
    ...
    try {
      ...
      final incident = Map<String, dynamic>.from(
        (data["incident"] ?? data) as Map,
      );
      final serverIncidentId = String(incident["_id"] ?? incident["id"] ?? "");
      final clientCreatedAt =
          DateTime.tryParse(String(payload["client_created_at"] ?? "")) ??
          DateTime.now();
      final cached = CachedIncidentRecord.fromServerJson(
        incident,
        localId: job.localId,
  ...
  Future<void> _reschedule(SyncQueueRecord job, {required String reason}) async {
  ...
  DateTime _nextRetryTime(int attemptCount) {
    if (attemptCount <= 0) return DateTime.now().add(const Duration(seconds: 1));
    if (attemptCount == 1) return DateTime.now().add(const Duration(seconds: 2));
    if (attemptCount == 2) return DateTime.now().add(const Duration(seconds: 4));
    if (attemptCount == 3) return DateTime.now().add(const Duration(seconds: 8));
