...
export const USSDSimulator = () => {
  ...
  const confirmEmergency = async () => {
    ...
    setTimeout(() => {
      setSms(
        `QuickReach: Your ${type} emergency in ${subCity} has been logged. Help is arriving!`,
      );
    }, 2000);
  };
          ...
          <div className="bg-[#94a3b8] aspect-[3/4] rounded-lg mb-6 p-4 flex flex-col items-center justify-center relative overflow-hidden">
            {ussdSession === null ? (
              ...
              <div className="w-full text-slate-900 font-mono text-sm">
                <p className="border-b border-slate-400 pb-1 mb-2 font-bold uppercase">
                  QuickReach Menu
                </p>
                <button
                  onClick={() => handleMenuOption("1")}
                  className="block w-full text-left hover:bg-black/5 p-1"
                >
                  1. Medical
                </button>
                <button
                ...
                <button
                  onClick={reset}
                  className="block w-full text-left hover:bg-black/5 p-1 mt-2 text-xs"
                >
                  0. Cancel
                </button>
              </div>
              ...
              <div className="w-full text-slate-900 font-mono text-sm">
                ...
                <button
                  onClick={() => handleLocationOption("1")}
                  className="block w-full text-left hover:bg-black/5 p-1"
                >
                  1. Arada
                </button>
                <button
                  onClick={() => handleLocationOption("2")}
                  className="block w-full text-left hover:bg-black/5 p-1"
                >
                  2. Bole
                </button>
                <button
                  onClick={() => handleLocationOption("3")}
                  className="block w-full text-left hover:bg-black/5 p-1"
                >
                  3. Akaki
                </button>
                <button
                  onClick={() => handleLocationOption("4")}
                  className="block w-full text-left hover:bg-black/5 p-1"
                >
                  4. Kolfe
                </button>
              ...
              <div className="w-full text-slate-900 font-mono text-sm text-center">
                <p className="mb-4">
                  Confirm {emergencyType} in {subCity}?
                </p>
                <div className="flex gap-2">
                  <button
                    onClick={confirmEmergency}
                    className="flex-1 bg-slate-900 text-white py-1 rounded"
                  >
                    Yes
                  </button>
                  <button
                    onClick={() => setUssdSession("PICK_LOCATION")}
                    className="flex-1 border border-slate-900 py-1 rounded"
                  >
                    No
                  </button>
                </div>
              ...
              <div className="w-full text-slate-900 font-mono text-sm text-center">
                ...
                <div className="text-[10px] space-y-1">
                  ...
                  <p>
                    Area: <span className="font-bold">{subCity}</span>
                  </p>
                </div>
                <button onClick={reset} className="mt-4 text-[10px] underline">
                  Back to Dial
                </button>
              </div>
            )}
            ...
            {sms && (
              <div className="absolute top-0 left-0 w-full p-2 animate-in slide-in-from-top duration-500">
                <div className="bg-white rounded p-2 shadow-lg border-l-4 border-blue-500">
                  ...
                  <p className="text-[9px] text-slate-800 leading-tight line-clamp-2">
                    {sms}
                  </p>
                </div>
              </div>
