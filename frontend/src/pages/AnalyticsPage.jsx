    ...
    <div className="min-h-screen bg-slate-950 text-white p-8">
      <div className="max-w-6xl mx-auto">
        ...
        <p className="text-slate-500 text-sm mb-8">
          Live competition-grade operations dashboard
        </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
          ...
          <Card
            ...
            value={`${metrics.dispatch_success_pct ?? 0}%`}
          />
          <Card
            icon={<Layers className="w-5 h-5" />}
            label="Collapse Rate"
            value={`${metrics.collapse_rate_pct ?? 0}%`}
          />
          <Card
            icon={<BarChart3 className="w-5 h-5" />}
            label="Avg Priority"
            value={metrics.average_priority ?? 0}
          />
        </div>
              ...
              <div key={h.name} className="mb-4">
                ...
                <div className="h-2 rounded-full bg-slate-800 overflow-hidden">
                  <div
                    ...
                    style={{ width: `${Math.min(100, h.load_pct)}%` }}
                  />
                </div>
              </div>
            ))}
