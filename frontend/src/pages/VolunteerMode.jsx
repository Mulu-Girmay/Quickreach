      ...
      <header className="flex justify-between items-center mb-8">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-2xl font-bold tracking-tight">
            QuickReach{" "}
            <span className="text-red-500 font-medium text-lg">Volunteer</span>
          </h1>
        </div>
        <div className="flex items-center gap-3 bg-slate-800 p-1.5 rounded-full border border-white/5">
          <span
            className={`text-xs font-bold px-3 py-1 rounded-full ${isOnline ? "bg-red-500/20 text-red-400" : "bg-slate-700 text-slate-400"}`}
          >
            {isOnline ? "ONLINE" : "OFFLINE"}
          </span>
          <button
            onClick={() => setIsOnline(!isOnline)}
            className={`w-12 h-6 rounded-full relative transition-colors ${isOnline ? "bg-red-600" : "bg-slate-600"}`}
          >
            <div
            ...
            {nearbyIncidents.length === 0 ? (
              ...
              nearbyIncidents.map((incident) => {
                ...
                return (
                  <div
                    key={id}
                    className="bg-slate-800 border border-red-500/30 rounded-3xl p-6 shadow-xl animate-pulse"
                  >
                    <div className="flex justify-between items-start mb-4">
                      <div>
                        <span className="text-xs font-bold text-red-400 uppercase tracking-widest bg-red-400/10 px-2 py-1 rounded mb-2 inline-block">
                          {incident.status}
                        </span>
                    ...
                    <button
                      ...
                      className="w-full bg-red-600 hover:bg-red-500 disabled:bg-slate-700 disabled:cursor-not-allowed text-white font-bold py-4 rounded-2xl transition-colors"
