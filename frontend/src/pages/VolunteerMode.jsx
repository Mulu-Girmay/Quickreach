...
export const VolunteerMode = () => {
  ...
  const fetchNearbyIncidents = async () => {
    try {
      ...
      if (location) {
        ...
        const withDist = allPending.map(inc => {
  ...
  const handleAcceptIncident = async (incident) => {
    try {
      await apiFetch(`/api/incidents/${incident.id}/volunteer-accept`, { method: 'POST' });
    ...
    if (isOnline) {
      const channel = supabase
        ...
        .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'incidents' }, payload => {
          // Re-fetch to calculate distance correctly
          fetchNearbyIncidents();
        })
        .subscribe();
      ...
      <header className="flex justify-between items-center mb-8">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-2xl font-bold tracking-tight">
            QuickReach <span className="text-blue-500 font-medium text-lg">Volunteer</span>
          </h1>
        </div>
      ...
      <main>
        {!isOnline ? (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            {profile?.name && <p className="text-xs text-slate-500 mt-4">Signed in as {profile.name}</p>}
          </div>
        ) : (
          <div className="space-y-6">
            ...
            <div className="flex items-center gap-2 mb-2">
              ...
              <h2 className="text-xl font-bold">Nearby Alerts ({nearbyIncidents.length})</h2>
            </div>
            ...
            {nearbyIncidents.length === 0 ? (
              <div className="bg-slate-800/30 border border-white/5 rounded-3xl p-10 text-center">
                ...
                {!location && <p className="text-xs text-orange-400 mt-2">Waiting for GPS...</p>}
              </div>
            ) : (
                ...
                <div key={incident.id} className="bg-slate-800 border border-blue-500/30 rounded-3xl p-6 shadow-xl animate-pulse">
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      ...
                      <p className="text-xs text-slate-400 mt-1">{new Date(incident.created_at).toLocaleTimeString()}</p>
                    ...
                    <div className="flex items-center gap-1 text-slate-400 text-sm font-bold">
                      ...
                      {incident.distance ? `${incident.distance.toFixed(1)} km` : 'Calculating...'}
                    </div>
                  </div>
                  <p className="text-slate-300 mb-6 text-sm">Emergency reported. First Responder assistance requested for immediate aid.</p>
                  <button 
                  ...
                  >
                    {acceptedIncidentIds.includes(incident.id) ? 'Accepted • View Location' : 'Accept & View Location'}
                  </button>
                </div>
              ))
            )}
