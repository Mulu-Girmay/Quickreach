...
export const PanicPage = () => {
  ...
  const lastLocationSyncRef = useRef({ ts: 0, lat: null, lng: null });
  const activeIncidentIdRef = useRef(null);
  const incidentTokenRef = useRef(null);
  ...
  const resetIncidentView = () => {
    ...
    setEta(null);
    setDistance(null);
    lastLocationSyncRef.current = { ts: 0, lat: null, lng: null };
  };

  const syncIncidentLocation = async (coords) => {
    if (!activeIncidentIdRef.current || !incidentTokenRef.current || !coords) return;

    const now = Date.now();
    const last = lastLocationSyncRef.current;
    const movedEnough =
      last.lat == null ||
      Math.abs(coords.lat - last.lat) > 0.0001 ||
      Math.abs(coords.lng - last.lng) > 0.0001;
    const dueByTime = now - last.ts >= 10000;

    if (!movedEnough && !dueByTime) return;

    try {
      await apiFetch(`/api/incidents/${activeIncidentIdRef.current}/location`, {
        method: 'PATCH',
        auth: false,
        headers: { 'x-incident-token': incidentTokenRef.current },
        body: { lat: coords.lat, lng: coords.lng }
      });
      lastLocationSyncRef.current = { ts: now, lat: coords.lat, lng: coords.lng };
    } catch (error) {
      // Keep SOS flow running even if a location sync attempt fails.
      console.error('Incident location sync failed:', error.message);
    }
  };
  ...
  useEffect(() => {
    ...
    const watchId = navigator.geolocation.watchPosition(
      (position) => {
        ...
        syncIncidentLocation(nextLocation);
      },
  ...
  useEffect(() => {
    ...
    incidentTokenRef.current = incidentAccessToken || null;
  ...
  const handlePanic = async () => {
    ...
    try {
      ...
      const createdIncident = incident.incident || incident;
      const createdToken = incident.incident_access_token || null;

      setActiveIncident(createdIncident);
      setIncidentAccessToken(createdToken);
      activeIncidentIdRef.current = createdIncident?.id || null;
      incidentTokenRef.current = createdToken;
      syncIncidentLocation(location);
  ...
  return (
    <div className="emergency-shell min-h-screen bg-slate-50 flex flex-col">
      {!activeIncident && (
        <Link to="/" className="fixed top-3 left-3 sm:top-6 sm:left-6 flex items-center gap-2 text-white hover:text-slate-200 transition-colors z-50 bg-red-700 px-3 py-1.5 sm:px-4 sm:py-2 rounded-full shadow-lg">
      ...
      <header className="bg-red-600 text-white p-4 sm:p-6 shadow-lg">
        <div className="max-w-4xl mx-auto flex flex-col sm:flex-row justify-between sm:items-center gap-3">
          ...
          <a href="tel:911" className="self-start sm:self-auto bg-white text-red-600 px-4 py-2 rounded-full font-bold flex items-center gap-2 hover:bg-slate-100 transition-colors">
      ...
      <main className="flex-1 max-w-4xl mx-auto w-full p-3 sm:p-6 grid gap-6 md:grid-cols-2">
        <div className="space-y-6">
          ...
          {!activeIncident ? (
            <button
              onClick={handlePanic}
              disabled={!location}
              className="w-full aspect-square max-w-[420px] mx-auto rounded-full bg-gradient-to-br from-red-500 to-red-700 text-white shadow-2xl shadow-red-500/30 flex flex-col items-center justify-center gap-2 hover:scale-105 active:scale-95 transition-transform disabled:opacity-50 disabled:grayscale"
            >
